...
# This variant first performs explicit cross-attention between image and metadata,
# then refines the fused token with a small transformer encoder.
...
class TransformerFusionModel(nn.Module):
    def __init__(self, num_classes, metadata_dim=4, embed_dim=256, nhead=4, num_layers=2, dropout=0.15):
        ...
        # Both modalities are projected into the same attention space.
        ...
        # Cross-attention lets each branch query information from the other one directly.
    ...
    def forward(self, images, metadata):
        ...
        return output, img_token, meta_token
...
if __name__ == '__main__':
    ...
    model = TransformerFusionModel(num_classes=num_classes).to(config.DEVICE)
    ...
    print("Transformer fusion model check passed!")
